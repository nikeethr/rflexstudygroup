...
def filter_by_cuisine(names_matching_price, cuisine_to_names, cuisines_list):
    ...
    for cuisine in cuisines_list:
        for name in cuisine_to_names[cuisine]:
            if name in names_matching_price and name not in names_matching:
                names_matching.append(name)
    return names_matching
...
def read_restaurants(file):
    ...
    for i in range(0, len(lines), 5):
        name, rating, price = lines[i:i+3]
        cuisine_list = lines[i+3].split(',')
...
if __name__ == '__main__':
    ...
    recommendations = recommend(file, '$', ['Malaysian', 'Chinese'])
